class Light extends Marking {
  ...
  draw(ctx) {
    ...
    switch (this.state) {
      case "green":
        green.draw(ctx, { size: this.height * 0.6, color: "#0F0" }); //Green light is solid
        break;
      case "yellow":
        yellow.draw(ctx, { size: this.height * 0.6, color: "#FF0" }); //Yellow light has a smaller base for visibility
        break;
      case "red":
        red.draw(ctx, { size: this.height * 0.6, color: "#F00" }); //Red light
        break;
    }
  }
}
